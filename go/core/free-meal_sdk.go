...
func (sdk *FreeMealSDK) OptionsMap() map[string]any {
...
func (sdk *FreeMealSDK) GetUtility() *Utility {
	return CopyUtility(sdk.utility)
}

func (sdk *FreeMealSDK) GetRootCtx() *Context {
	return sdk.rootctx
}

func (sdk *FreeMealSDK) Prepare(fetchargs map[string]any) (map[string]any, error) {
	utility := sdk.utility
...
func (sdk *FreeMealSDK) Direct(fetchargs map[string]any) (map[string]any, error) {
...
func (sdk *FreeMealSDK) opAllowed(op string) bool {
	allowOp, _ := vs.GetPath([]any{"allow", "op"}, sdk.options).(string)
	return strings.Contains(allowOp, op)
}

func (sdk *FreeMealSDK) opDenied(op string) map[string]any {
	allowOp, _ := vs.GetPath([]any{"allow", "op"}, sdk.options).(string)
	return map[string]any{
		"ok": false,
		"err": fmt.Errorf("FreeMealSDK: %s: operation not allowed by"+
...
// gate by passing it.
func (sdk *FreeMealSDK) rawRequest(fetchargs map[string]any) (map[string]any, error) {
	utility := sdk.utility
...
func (sdk *FreeMealSDK) Graphql(
	...
	if 0 < len(errors) {
		...
		res["ok"] = false
		res["err"] = fmt.Errorf("FreeMealSDK: graphql: %s", msg)
		res["graphql"] = errors
	}
