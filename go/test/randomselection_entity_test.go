...
func TestRandomselectionEntity(t *testing.T) {
	...
	t.Run("stream", func(t *testing.T) {
		...
		var seen []any
		for item := range base.Randomselection(nil).Stream("list", nil, nil) {
		...
		if hasStreaming {
			...
			var got []any
			for item := range streamSdk.Randomselection(nil).Stream("list", nil, nil) {
	...
	t.Run("basic", func(t *testing.T) {
		...
		if setup.syntheticOnly {
			t.Skip("live entity test uses synthetic IDs from fixture — set FREE_MEAL_TEST_RANDOMSELECTION_ENTID JSON to run live")
			return
		}
		client := setup.client
...
func randomselectionBasicSetup(extra map[string]any) *entityTestSetup {
	...
	entidEnvRaw := os.Getenv("FREE_MEAL_TEST_RANDOMSELECTION_ENTID")
