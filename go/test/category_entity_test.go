...
func TestCategoryEntity(t *testing.T) {
	...
	t.Run("basic", func(t *testing.T) {
		...
		if setup.syntheticOnly {
			t.Skip("live entity test uses synthetic IDs from fixture — set FREEMEAL_TEST_CATEGORY_ENTID JSON to run live")
			return
		}
		client := setup.client
...
func categoryBasicSetup(extra map[string]any) *entityTestSetup {
	...
	entidEnvRaw := os.Getenv("FREEMEAL_TEST_CATEGORY_ENTID")
	...
	idmapResolved := core.ToMapAny(env["FREEMEAL_TEST_CATEGORY_ENTID"])
