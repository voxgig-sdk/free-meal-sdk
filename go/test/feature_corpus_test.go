...
//
// NewFreeMealSDK, not TestSDK: the `test` feature is transport: 'base' and
// REPLACES the transport, so a client in test mode would shadow the script.
func fcClient(kase map[string]any) *sdk.FreeMealSDK {
	res, _ := kase["res"].([]any)
	opts := map[string]any{
	...
	return sdk.NewFreeMealSDK(opts)
}
...
func fcCandidates(client *sdk.FreeMealSDK) []fcOp {
	out := []fcOp{}
...
func fcInvoke(client *sdk.FreeMealSDK, op fcOp, ctrl map[string]any) error {
	cv := reflect.ValueOf(client)
	ct := cv.Type()
	mapType := reflect.TypeOf(map[string]any{})
...
// new go here.
func fcRecord(client *sdk.FreeMealSDK, name string) any {
