...
import (
	...
	omni "github.com/voxgig-sdk/free-meal-sdk/go/test/omni"
...
func omniProvider(client any) *omni.Provider {
	provider := &omni.Provider{
		...
		Errify: func(err any) any {
			if sdkerr, is := err.(*sdk.FreeMealError); is {
				out := map[string]any{
					"name":    "FreeMealError",
...
func omniCtx(arg any, client *sdk.FreeMealSDK, utility *sdk.Utility) *sdk.Context {
	...
	if provider, is := ctxmap["client"].(*omni.Provider); is {
		if live, is := defproviderclient(provider).(*sdk.FreeMealSDK); is {
			client = live
