...
func TestLatestEntity(t *testing.T) {
	...
	t.Run("stream", func(t *testing.T) {
		...
		var seen []any
		for item := range base.Latest(nil).Stream("list", nil, nil) {
		...
		if hasStreaming {
			...
			var got []any
			for item := range streamSdk.Latest(nil).Stream("list", nil, nil) {
	...
	t.Run("basic", func(t *testing.T) {
		...
		if setup.syntheticOnly {
			t.Skip("live entity test uses synthetic IDs from fixture — set FREE_MEAL_TEST_LATEST_ENTID JSON to run live")
			return
		}
		client := setup.client
...
func latestBasicSetup(extra map[string]any) *entityTestSetup {
	...
	entidEnvRaw := os.Getenv("FREE_MEAL_TEST_LATEST_ENTID")
	...
	idmapResolved := core.ToMapAny(env["FREE_MEAL_TEST_LATEST_ENTID"])
