...
[MCP](https://modelcontextprotocol.io) server exposing the FreeMeal SDK as
two agent tools — `free-meal_list` and `free-meal_load` — built on the
...
```sh
# 1. Build a native binary (-> dist/<os>-<arch>/free-meal-mcp)
make build

# 2. Provide credentials via the environment
export FREE_MEAL_APIKEY=sk_live_xxx
...
```jsonc
// free-meal_list: first page of records
{ "entity": "category" }
{ "entity": "category", "query": { } }

// free-meal_load: one record by id
...
1. **Build** the server from this `go-mcp/` directory:
   ...
   ```sh
   make build          # -> dist/<os>-<arch>/free-meal-mcp
   ```
...
3. **Install it into Claude Code** (stdio transport):
   ...
   ```sh
   claude mcp add --scope user free-meal \
     -- "$PWD"/dist/*/free-meal-mcp -transport stdio
   ```

4. **Restart Claude Code.** The `free-meal_list` and `free-meal_load` tools now appear
   in new sessions. Ask the agent to *"list category using free-meal"*
   and it calls `free-meal_list` with `{"entity":"category"}`.
...
```sh
export FREE_MEAL_APIKEY=sk_live_xxx            # API key
export FREE_MEAL_BASE=https://api.example.com  # optional: override the API base URL
```
...
| Tool | Args | Returns |
|------|------|---------|
| `free-meal_list` | `entity` (required), `query` (optional map) | First page of records as JSON |
| `free-meal_load` | `entity` (required), `query` = `{id:N}` | Single record as JSON |
...
| Variable | Purpose |
|----------|---------|
| `FREE_MEAL_APIKEY` | API key sent with every request. |
| `FREE_MEAL_BASE` | Optional override of the API base URL. |
