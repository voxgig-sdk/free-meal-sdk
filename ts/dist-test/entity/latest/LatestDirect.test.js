...
(0, node_test_1.describe)('LatestDirect', async () => {
    ...
    (0, node_test_1.test)('direct-list-latest', async (t) => {
        if (liveScenariosActive()) {
            t.skip('Covered by live operation scenarios');
            return;
        }
        const setup = directSetup([{ id: 'direct01' }, { id: 'direct02' }]);
        if ((0, utility_1.maybeSkipControl)(t, 'direct', 'direct-list-latest', setup.live))
            return;
...
function directSetup(mockres) {
    ...
    if (live) {
        ...
        let idmap = env['FREE_MEAL_TEST_LATEST_ENTID'];
