...
//     (\`new FreeMealSDK(...)\` / \`FreeMealSDK.test(...)\` become
//     \`FreeMealSDK.test({ entity: ... })\`), transpiled to JS, and run
...
const TEST_SEED = {"entity":{"category":{"test01":{"id":"test01"}},"filter":{"test01":{"id":"test01"}},"latest":{"test01":{"id":"test01"}},"list":{"test01":{"id":"test01"}},"lookup":{"test01":{"id":"test01"}},"random":{"test01":{"id":"test01"}},"randomselection":{"test01":{"id":"test01"}},"search":{"test01":{"id":"test01"}}}}
...
// rewrite real client construction (\`new FreeMealSDK(...)\` and any
// \`FreeMealSDK.test(...)\`) to a seeded test-mode client, and give
...
function rewriteForRun(code: string): string {
  ...
  out = out.replace(/new\s+FreeMealSDK\s*\([^)]*\)/g, () => SEEDED_CTOR)
  out = out.replace(/FreeMealSDK\.test\s*\([^)]*\)/g, () => SEEDED_CTOR)
