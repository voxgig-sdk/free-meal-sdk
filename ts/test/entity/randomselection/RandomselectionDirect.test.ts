...
describe('RandomselectionDirect', async () => {
  ...
  test('direct-list-randomselection', async (t: any) => {
    if (liveScenariosActive()) { t.skip('Covered by live operation scenarios'); return }
    const setup = directSetup([{ id: 'direct01' }, { id: 'direct02' }])
    if (maybeSkipControl(t, 'direct', 'direct-list-randomselection', setup.live)) return
...
function directSetup(mockres?: any) {
  ...
  if (live) {
    ...
    let idmap: any = env['FREE_MEAL_TEST_RANDOMSELECTION_ENTID']
