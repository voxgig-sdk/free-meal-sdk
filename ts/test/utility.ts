...
function envOverride(m: Record<string, any>) {
  if (
    'TRUE' === process.env.FREEMEAL_TEST_LIVE ||
    'TRUE' === process.env.FREEMEAL_TEST_OVERRIDE
  ) {
  ...
  m.FREEMEAL_TEST_EXPLAIN = process.env.FREEMEAL_TEST_EXPLAIN || m.FREEMEAL_TEST_EXPLAIN
