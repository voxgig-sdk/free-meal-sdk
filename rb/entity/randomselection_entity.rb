...
class RandomselectionEntity
  ...
  def make
    opts = @_entopts.dup
    RandomselectionEntity.new(@_client, opts)
  end

  def data_set(args)
    if args
      @_data = FreeMealHelpers.to_map(VoxgigStruct.clone(args)) || {}
      @_utility.feature_hook.call(@_entctx, "SetData")
    end
  end

  # @return [Randomselection, Hash] the current Randomselection data
  def data_get
  ...
  def match_set(args)
    if args
      @_match = FreeMealHelpers.to_map(VoxgigStruct.clone(args)) || {}
      @_utility.feature_hook.call(@_entctx, "SetMatch")
    end
  end

  # @return [Hash] the current match filter (any subset of Randomselection fields)
  def match_get
  ...
  #
  # @param reqmatch [RandomselectionListMatch, Hash, nil] match filter (any subset of
  #   Randomselection fields); defaults to nil, treated as an empty match that lists all.
  # @param ctrl [Object, nil] optional per-call control
  # @return [Array<Randomselection>, Array] the matching Randomselection items; raises FreeMealError on failure
