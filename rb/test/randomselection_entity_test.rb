...
class RandomselectionEntityTest < Minitest::Test
  ...
  def test_stream
    ...
    base = FreeMealSDK.test(seed, nil)
    seen = base.Randomselection(nil).stream("list", nil, nil).to_a
    ...
    if cfg["feature"].is_a?(Hash) && cfg["feature"].key?("streaming")
      ...
      got = []
      sdk.Randomselection(nil).stream("list", nil, nil).each do |item|
        if item.is_a?(Array)
          got.concat(item)
        else
          got << item
  ...
  def test_basic_flow
    setup = randomselection_basic_setup(nil)
    # Per-op sdk-test-control.json skip.
    _live = setup[:live] || false
    ["list"].each do |_op|
      _should_skip, _reason = Runner.is_control_skipped("entityOp", "randomselection." + _op, _live ? "live" : "unit")
      if _should_skip
    ...
    if setup[:synthetic_only]
      skip "live entity test uses synthetic IDs from fixture — set FREE_MEAL_TEST_RANDOMSELECTION_ENTID JSON to run live"
      return
    end
    client = setup[:client]
    ...
    randomselection_ref01_data = nil
    if randomselection_ref01_data_raw.length > 0
...
def randomselection_basic_setup(extra)
  Runner.load_env_local

  entity_data_file = File.join(__dir__, "..", "..", ".sdk", "test", "entity", "randomselection", "RandomselectionTestData.json")
  ...
  entid_env_raw = ENV["FREE_MEAL_TEST_RANDOMSELECTION_ENTID"]
