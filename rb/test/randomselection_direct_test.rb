...
class RandomselectionDirectTest < Minitest::Test
  def test_direct_list_randomselection
    setup = randomselection_direct_setup([
      { "id" => "direct01" },
      { "id" => "direct02" },
    ])
    _should_skip, _reason = Runner.is_control_skipped("direct", "direct-list-randomselection", setup[:live] ? "live" : "unit")
    if _should_skip
...
def randomselection_direct_setup(mockres)
