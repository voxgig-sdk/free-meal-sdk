...
module FreeMealUtilities
  MakeRequest = ->(ctx) {
    ...
    if fetch_err
      ...
      response = FreeMealResponse.new({ "err" => ctx.make_error("request_no_response", "response: undefined") })
    elsif fetched.is_a?(Hash)
      response = FreeMealResponse.new(fetched)
    else
