...
class FreeMealTestRunner:
    ...
    def load_env_local():
        ...
        for line in content.splitlines():
            ...
            FreeMealTestRunner._env[key] = val

    @staticmethod
    def getenv(key):
        val = FreeMealTestRunner._env.get(key)
        if val is not None:
            return val
    ...
    @staticmethod
    def env_override(m):
        live = FreeMealTestRunner.getenv("PROJECTENV_TEST_LIVE")
        override = FreeMealTestRunner.getenv("PROJECTENV_TEST_OVERRIDE")

        if live == "TRUE" or override == "TRUE":
            for key in list(m.keys()):
                envval = FreeMealTestRunner.getenv(key)
        ...
        explain = FreeMealTestRunner.getenv("PROJECTENV_TEST_EXPLAIN")
    ...
    def load_test_control():
        ...
        """
        if FreeMealTestRunner._test_control is not None:
            return FreeMealTestRunner._test_control
        ctrl_path = os.path.join(os.path.dirname(__file__), "sdk-test-control.json")
        try:
            with open(ctrl_path, "r") as f:
                FreeMealTestRunner._test_control = json.load(f)
        except (FileNotFoundError, IOError, ValueError):
            FreeMealTestRunner._test_control = {
                "version": 1,
                "test": {"skip": {
    ...
    def is_control_skipped(kind, name, mode):
        ...
        ctrl = FreeMealTestRunner.load_test_control()
...
def load_env_local():
    FreeMealTestRunner.load_env_local()


def env_override(m):
    return FreeMealTestRunner.env_override(m)


def entity_data(v):
    return FreeMealTestRunner.entity_data(v)


def entity_list_to_data(lst):
    return FreeMealTestRunner.entity_list_to_data(lst)


def is_control_skipped(kind, name, mode):
    return FreeMealTestRunner.is_control_skipped(kind, name, mode)


def load_test_control():
    return FreeMealTestRunner.load_test_control()


def live_client_options():
    return FreeMealTestRunner.live_client_options()


def live_delay_ms():
    return FreeMealTestRunner.live_delay_ms()
