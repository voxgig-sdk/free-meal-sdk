...
{
    private static function utility(): FreeMealUtility
    {
        return new FreeMealUtility();
    }
    ...
    private static function ctx(array $over = []): FreeMealContext
    {
        $utility = $over['utility'] ?? self::utility();
        $client = $over['client'] ?? new PlClient(['base' => 'http://h']);
        $ctx = new FreeMealContext([
            'client' => $client,
            'utility' => $utility,
        ], null);
        $ctx->op = new FreeMealOperation(['name' => 'load', 'entity' => 'x']);
        foreach ($over as $k => $v) {
    ...
    {
        return ($err instanceof FreeMealError) ? $err->sdk_code : '';
    }
    ...
    public function test_make_point_rejects_a_disallowed_operation(): void
    {
        ...
        $ctx->op = new FreeMealOperation(['name' => 'nope', 'entity' => 'x']);
        [$point, $err] = FreeMealMakePoint::call($ctx);
    ...
    public function test_make_point_rejects_an_operation_with_no_endpoints(): void
    {
        ...
        $ctx->op = new FreeMealOperation(['name' => 'load', 'entity' => 'x', 'points' => []]);
        [$point, $err] = FreeMealMakePoint::call($ctx);
        $this->assertNull($point);
    ...
    public function test_make_point_returns_the_single_point(): void
    {
        ...
        $ctx = self::ctx(['options' => ['allow' => ['op' => 'load,list,create,update,remove']]]);
        $ctx->op = new FreeMealOperation(['name' => 'load', 'entity' => 'x', 'points' => [$point]]);
        [$got, $err] = FreeMealMakePoint::call($ctx);
        $this->assertNull($err);
    ...
    {
        $preset = new FreeMealSpec(['method' => 'GET']);
        $ctx = self::ctx();
        $ctx->out['spec'] = $preset;
        [$got, $err] = FreeMealMakeSpec::call($ctx);
        $this->assertNull($err);
        $this->assertSame($preset, $got);
    }
    ...
    public function test_make_response_guards_missing_spec_response_result(): void
    {
        $ctx = self::ctx([
            'spec' => null,
            'response' => new FreeMealResponse([]),
            'result' => new FreeMealResult([]),
        ]);
        [, $err] = FreeMealMakeResponse::call($ctx);
        $this->assertSame('response_no_spec', self::code($err));

        $ctx = self::ctx([
            'spec' => new FreeMealSpec([]),
            'response' => null,
            'result' => new FreeMealResult([]),
        ]);
        [, $err] = FreeMealMakeResponse::call($ctx);
    ...
    {
        $ctx = self::ctx([
            'spec' => new FreeMealSpec(['step' => 's']),
            'response' => new FreeMealResponse(self::resp(404, null, ['x-a' => '1'])),
            'result' => new FreeMealResult([]),
        ]);
        [, $err] = FreeMealMakeResponse::call($ctx);
        $this->assertNull($err);
    ...
    public function test_make_response_2xx_parses_the_body_and_marks_ok(): void
    {
        $ctx = self::ctx([
            'spec' => new FreeMealSpec(['step' => 's']),
            'response' => new FreeMealResponse(self::resp(200, ['v' => 1])),
            'result' => new FreeMealResult([]),
        ]);
        [, $err] = FreeMealMakeResponse::call($ctx);
        $this->assertNull($err);
    ...
    public function test_make_response_records_to_ctrl_explain_when_explain_is_on(): void
    {
        $ctx = self::ctx([
            'spec' => new FreeMealSpec(['step' => 's']),
            'response' => new FreeMealResponse(self::resp(200, ['v' => 2])),
            'result' => new FreeMealResult([]),
        ]);
        $ctx->ctrl->explain = ['on' => true];
        FreeMealMakeResponse::call($ctx);
    ...
    {
        $preset = new FreeMealResponse(self::resp(299));
        $ctx = self::ctx([
            'spec' => new FreeMealSpec([]),
            'response' => new FreeMealResponse([]),
            'result' => new FreeMealResult([]),
        ...
        [$got, $err] = FreeMealMakeResponse::call($ctx);
        $this->assertNull($err);
        $this->assertSame($preset, $got);
    }
    ...
    {
        $ctx = self::ctx(['spec' => null, 'result' => new FreeMealResult([])]);
        [, $err] = FreeMealMakeResult::call($ctx);
        $this->assertSame('result_no_spec', self::code($err));

        $ctx = self::ctx(['spec' => new FreeMealSpec([]), 'result' => null]);
        [, $err] = FreeMealMakeResult::call($ctx);
    ...
    public function test_make_result_list_op_wraps_resdata_into_entity_instances(): void
    {
        ...
        $ctx = self::ctx([
            'entity' => $entity,
            'spec' => new FreeMealSpec(['step' => 's']),
            'result' => new FreeMealResult(['ok' => true, 'resdata' => [['a' => 1], ['a' => 2]]]),
        ]);
        $ctx->op = new FreeMealOperation(['name' => 'list', 'entity' => 'x']);
        [$result, $err] = FreeMealMakeResult::call($ctx);
        $this->assertNull($err);
    ...
    public function test_make_result_empty_list_yields_empty_resdata(): void
    {
        ...
        $ctx = self::ctx([
            'entity' => $entity,
            'spec' => new FreeMealSpec(['step' => 's']),
            'result' => new FreeMealResult(['ok' => true, 'resdata' => []]),
        ]);
        $ctx->op = new FreeMealOperation(['name' => 'list', 'entity' => 'x']);
        [$result, $err] = FreeMealMakeResult::call($ctx);
        $this->assertNull($err);
    ...
    {
        $preset = new FreeMealResult(['ok' => true]);
        $ctx = self::ctx([
            'spec' => new FreeMealSpec([]),
            'result' => new FreeMealResult([]),
        ]);
        $ctx->out['result'] = $preset;
        [$got, $err] = FreeMealMakeResult::call($ctx);
        $this->assertNull($err);
        $this->assertSame($preset, $got);
    }
    ...
    public function test_make_request_guards_a_missing_spec(): void
    {
        ...
        [, $err] = FreeMealMakeRequest::call($ctx);
    ...
    {
        $utility = self::utility();
        $boom = new FreeMealError('boom', 'boom');
        $utility->fetcher = function (FreeMealContext $_c, string $_u, array $_f) use ($boom): array {
            return [null, $boom];
        };
        $ctx = self::ctx([
            'utility' => $utility,
            'spec' => new FreeMealSpec(['step' => 's', 'method' => 'GET', 'base' => 'http://h', 'parts' => ['a']]),
        ]);
        [$response, $err] = FreeMealMakeRequest::call($ctx);
        $this->assertNull($err);
    ...
    {
        $utility = self::utility();
        $utility->fetcher = function (FreeMealContext $_c, string $_u, array $_f): array {
            return [null, null];
        };
        $ctx = self::ctx([
            'utility' => $utility,
    ...
    public function test_make_request_a_normal_transport_response_is_wrapped(): void
    {
        ...
        [$response, $err] = FreeMealMakeRequest::call($ctx);
        $this->assertNull($err);
        $this->assertInstanceOf(FreeMealResponse::class, $response);
    ...
    {
        $utility = self::utility();
        $utility->make_fetch_def = function (FreeMealContext $c): array {
            return [null, $c->make_error('fetchdef_boom', 'boom')];
        };
        $ctx = self::ctx([
            'utility' => $utility,
            'spec' => new FreeMealSpec(['step' => 's', 'method' => 'GET']),
        ]);
        [$response, $err] = FreeMealMakeRequest::call($ctx);
        $this->assertNull($err);
        $this->assertNotNull($response->err);
    ...
    {
        $preset = new FreeMealResponse(self::resp(201));
        $ctx = self::ctx(['spec' => new FreeMealSpec([])]);
        $ctx->out['request'] = $preset;
        [$got, $err] = FreeMealMakeRequest::call($ctx);
        $this->assertNull($err);
        $this->assertSame($preset, $got);
    }
    ...
    {
        $ctx = self::ctx(['spec' => null]);
        [, $err] = FreeMealMakeFetchDef::call($ctx);
    ...
    public function test_make_fetch_def_serialises_body_and_inits_missing_result(): void
    {
        ...
        [$fetchdef, $err] = FreeMealMakeFetchDef::call($ctx);
        $this->assertNull($err);
    ...
    {
        $ctx = self::ctx(['result' => new FreeMealResult(['ok' => true, 'resdata' => 42])]);
        $this->assertSame(42, FreeMealDone::call($ctx));
    }

    public function test_done_raises_the_error_when_not_ok(): void
    {
        $ctx = self::ctx(['result' => new FreeMealResult(['ok' => false])]);
        $this->expectException(FreeMealError::class);
        FreeMealDone::call($ctx);
    }

    public function test_make_error_returns_resdata_when_ctrl_throw_is_false(): void
    {
        $ctx = self::ctx(['result' => new FreeMealResult(['ok' => false, 'resdata' => 'fallback'])]);
        $ctx->ctrl->throw_err = false;
        $this->assertSame('fallback', FreeMealMakeError::call($ctx, null));
    }
    ...
    public function test_make_error_records_to_ctrl_explain(): void
    {
        ...
        $ctx->ctrl->throw_err = false;
        $ctx->ctrl->explain = ['on' => true];
        FreeMealMakeError::call($ctx, null);
    ...
    public function test_feature_add_appends_in_call_order(): void
    {
        ...
        $a = new FreeMealBaseFeature();
        $b = new FreeMealBaseFeature();
        FreeMealFeatureAdd::call($ctx, $a);
        FreeMealFeatureAdd::call($ctx, $b);
        $this->assertSame([$a, $b], $client->features);
    }

    private static function named_feature(string $name): FreeMealBaseFeature
    {
        $f = new FreeMealBaseFeature();
        $f->name = $name;
        return $f;
    }
    ...
    public function test_feature_add_ordering_before_after_replace(): void
    {
        ...
        FreeMealFeatureAdd::call($ctx, $miss);
    ...
    private static function auth_ctx(array $options, ?array $headers): FreeMealContext
    {
        $client = new PlClient($options);
        return self::ctx([
            'client' => $client,
            'spec' => $headers === null ? null : new FreeMealSpec(['headers' => $headers]),
        ]);
    }
    ...
    public function test_prepare_auth_guards_a_missing_spec(): void
    {
        ...
        [, $err] = FreeMealPrepareAuth::call($ctx);
    ...
    {
        $ctx = self::ctx([
            'response' => new FreeMealResponse(['headers' => null]),
            'result' => new FreeMealResult([]),
        ]);
        FreeMealResultHeaders::call($ctx);
