...
class TestRandomselectionEntity:
    ...
    def test_should_stream(self):
        ...
        base = FreeMealSDK.test(seed, None)
        seen = list(base.Randomselection(None).stream("list", None, None))
        assert len(seen) == 3
        ...
        if isinstance(cfg.get("feature"), dict) and "streaming" in cfg["feature"]:
            sdk = FreeMealSDK.test(
                seed, {"feature": {"streaming": {"active": True}}})
            got = []
            for item in sdk.Randomselection(None).stream("list", None, None):
    ...
    def test_should_run_basic_flow(self):
        ...
        if setup.get("synthetic_only"):
            pytest.skip("live entity test uses synthetic IDs from fixture — "
                        "set FREE_MEAL_TEST_RANDOMSELECTION_ENTID JSON to run live")
        client = setup["client"]
...
def _randomselection_basic_setup(extra):
    ...
    idmap_resolved = helpers.to_map(
        env.get("FREE_MEAL_TEST_RANDOMSELECTION_ENTID"))
    ...
    _live = env.get("FREE_MEAL_TEST_LIVE") == "TRUE"
    return {
        "client": client,
        "data": entity_data,
        "idmap": idmap_resolved,
        "env": env,
        "explain": env.get("FREE_MEAL_TEST_EXPLAIN") == "TRUE",
        "live": _live,
