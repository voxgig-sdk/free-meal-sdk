...
class TestRandomselectionDirect:

    def test_should_direct_list_randomselection(self):
        setup = _randomselection_direct_setup([
            {"id": "direct01"},
            {"id": "direct02"},
        ])
        _skip, _reason = runner.is_control_skipped("direct", "direct-list-randomselection", "live" if setup["live"] else "unit")
        if _skip:
...
def _randomselection_direct_setup(mockres):
