...
class TestLatestDirect:
    ...
    def test_should_direct_list_latest(self):
        setup = _latest_direct_setup([
            {"id": "direct01"},
            {"id": "direct02"},
        ])
        _skip, _reason = runner.is_control_skipped("direct", "direct-list-latest", "live" if setup["live"] else "unit")
        if _skip:
