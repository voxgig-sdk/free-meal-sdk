...
describe("LatestEntity", function()
  ...
  it("should stream", function()
    ...
    local seen = {}
    for item in base:Latest(nil):stream("list", nil, nil) do
    ...
    if type(config.feature) == "table" and config.feature.streaming ~= nil then
      ...
      local got = {}
      for item in streamsdk:Latest(nil):stream("list", nil, nil) do
  ...
  it("should run basic flow", function()
    ...
    if setup.synthetic_only then
      pending("live entity test uses synthetic IDs from fixture — set FREE_MEAL_TEST_LATEST_ENTID JSON to run live")
      return
    end
    ...
    local latest_ref01_match = {}

    local latest_ref01_list_result, err = latest_ref01_ent:list(latest_ref01_match, nil)
    assert.is_nil(err)
    assert.is_table(latest_ref01_list_result)
...
function latest_basic_setup(extra)
  ...
  local entid_env_raw = os.getenv("FREE_MEAL_TEST_LATEST_ENTID")
