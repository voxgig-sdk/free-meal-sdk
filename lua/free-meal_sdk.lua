...
local FreeMealSDK = {}
FreeMealSDK.__index = FreeMealSDK
...
FreeMealSDK._make_feature = _make_feature


function FreeMealSDK.new(options)
  local self = setmetatable({}, FreeMealSDK)
  self.mode = "live"
  self.features = {}
  self.options = nil
...
function FreeMealSDK:options_map()
...
function FreeMealSDK:get_utility()
  return Utility.copy(self._utility)
end


function FreeMealSDK:get_root_ctx()
  return self._rootctx
end


function FreeMealSDK:prepare(fetchargs)
...
function FreeMealSDK:direct(fetchargs)
...
-- by passing it.
function FreeMealSDK:_raw_request(fetchargs)
  local utility = self._utility
...
function FreeMealSDK:graphql(query, variables, ctrl)
  ...
  if type(errors) == "table" and 0 < #errors then
    ...
    res.ok = false
    res.err = "FreeMealSDK: graphql: " .. msg
    res.graphql = errors
  end
...
function FreeMealSDK.test(testopts, sdkopts)
  sdkopts = sdkopts or {}
  ...
  local sdk = FreeMealSDK.new(sdkopts)
  sdk.mode = "test"
