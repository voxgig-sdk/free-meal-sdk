...
class FtCtrl extends FreeMealControl
{
...
class FtClient
{
    ...
    private FreeMealUtility $utility;

    public function __construct(FreeMealUtility $utility, array $options)
    {
    ...
    public function get_utility(): FreeMealUtility
    {
        return $this->utility;
    }
}
...
class FtRecorder
{
    ...
    {
        return function (FreeMealContext $_ctx, string $url, array $fetchdef): array {
...
class FtHarness
{
    ...
    public FreeMealContext $rootctx;
    ...
    public function __construct(
        ...
        $utility->fetcher = $server ?? function (FreeMealContext $_ctx, string $_url, array $fetchdef): array {
        ...
        foreach ($features as $fspec) {
            ...
            $f = FreeMealConfig::make_feature($name);
    ...
    private static function buildUrl(FreeMealSpec $spec): string
    {
        $keys = [];
    ...
    public function op(array $o = []): array
    {
        ...
        if (($ctx->out['point'] ?? null) instanceof FreeMealError) {
        ...
        if (is_array($fetched)) {
            $ctx->response = new FreeMealResponse($fetched);
        }
    ...
    private function fail(FreeMealContext $ctx, mixed $err): array
    {
...
class FeatureTest extends TestCase
{
    ...
    public function test_retry_retries_a_transport_error_until_exhausted(): void
    {
        ...
        $clock = new FtClock();
        $n = 0;
        $server = function (FreeMealContext $ctx, string $_url, array $_fd) use (&$n): array {
            $n++;
