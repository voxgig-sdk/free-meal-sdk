...
class RandomEntityTest extends TestCase
{
    ...
    public function test_basic_flow(): void
    {
        ...
        if (!empty($setup["synthetic_only"])) {
            $this->markTestSkipped("live entity test uses synthetic IDs from fixture — set FREEMEAL_TEST_RANDOM_ENTID JSON to run live");
            return;
        }
        $client = $setup["client"];
...
function random_basic_setup($extra)
{
    ...
    $entid_env_raw = getenv("FREEMEAL_TEST_RANDOM_ENTID");
    $idmap_overridden = $entid_env_raw !== false && str_starts_with(trim($entid_env_raw), "{");

    $env = Runner::env_override([
        "FREEMEAL_TEST_RANDOM_ENTID" => $idmap,
    ...
    $idmap_resolved = Helpers::to_map(
        $env["FREEMEAL_TEST_RANDOM_ENTID"]);
