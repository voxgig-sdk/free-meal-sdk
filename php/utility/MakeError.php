...
class FreeMealMakeError
{
    public static function call(?FreeMealContext $ctx, mixed $err): mixed
    {
        if ($ctx === null) {
            require_once __DIR__ . '/../core/Context.php';
            $ctx = new FreeMealContext([], null);
        }
        $op = $ctx->op ?? new FreeMealOperation([]);
        $opname = $op->name;
        if ($opname === '' || $opname === '_') {
            $opname = 'unknown operation';
        }

        $result = $ctx->result ?? new FreeMealResult([]);
        $result->ok = false;
        ...
        $errmsg = ($err instanceof FreeMealError) ? $err->msg : (string)$err;
        ...
        $sdk_err = new FreeMealError('', $msg, $ctx);
        $sdk_err->result = ($ctx->utility->clean)($ctx, $result);
        $sdk_err->spec = ($ctx->utility->clean)($ctx, $spec);
        if ($err instanceof FreeMealError) {
