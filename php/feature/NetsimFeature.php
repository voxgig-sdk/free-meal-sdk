...
class FreeMealNetsimFeature extends FreeMealBaseFeature
{
    ...
    private function _simulate(FreeMealContext $ctx, string $url, array $fetchdef, callable $inner): array
    {
        $opts = $this->options ?? [];
        $this->calls++;
        $call = $this->calls;
    ...
    private function _track(FreeMealContext $ctx, array $applied): void
    {
        $client = $this->client;
