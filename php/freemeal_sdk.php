...
class FreeMealSDK
{
    ...
    public function __construct(array $options = [])
    {
        ...
        $feature_opts = FreeMealHelpers::to_map(Struct::getprop($this->options, "feature"));
        if ($feature_opts) {
            $featureorder = Struct::getpath($this->options, "__derived__.featureorder");
            if (is_array($featureorder)) {
                foreach ($featureorder as $fname) {
                    $fopts = FreeMealHelpers::to_map($feature_opts[$fname] ?? null);
                    if ($fopts && isset($fopts["active"]) && $fopts["active"] === true) {
                        ...
                        if (!FreeMealFeatures::has_feature($fname)) {
                        ...
                        ($utility->feature_add)($this->_rootctx, FreeMealFeatures::make_feature($fname));
                    }
                }
            }
        }
    ...
    {
        return FreeMealUtility::copy($this->_utility);
    }
    ...
    public function prepare(array $fetchargs = []): mixed
    {
        ...
        $ctrl = FreeMealHelpers::to_map(Struct::getprop($fetchargs, "ctrl")) ?? [];
        ...
        $params = FreeMealHelpers::to_map(Struct::getprop($fetchargs, "params")) ?? [];
        $query = FreeMealHelpers::to_map(Struct::getprop($fetchargs, "query")) ?? [];
    ...
    private function op_denied(string $op): array
    {
        ...
        return [
            "ok" => false,
            "err" => new FreeMealError($op . "_allow",
    ...
    private function raw_request(array $fetchargs = []): mixed
    {
        ...
        if (is_array($fetched)) {
            $status = FreeMealHelpers::to_int(Struct::getprop($fetched, "status"));
    ...
    public function graphql(string $query, ?array $variables = null, ?array $ctrl = null): mixed
    {
        ...
        if (is_array($errors) && 0 < count($errors)) {
            ...
            $res["ok"] = false;
            $res["err"] = new FreeMealError("graphql_error",
                "FreeMealSDK: graphql: " . $msg);
            $res["graphql"] = $errors;
        }
    ...
    // Canonical facade: $client->Randomselection()->list() / ->load(["id" => ...]).
    // PHP method names are case-insensitive, so lowercase $client->randomselection()
    // resolves here too.
