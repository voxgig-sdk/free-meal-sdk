...
boru-driven command-line client **and** interactive REPL for the FreeMeal
...
```sh
# 1. Build a native binary (-> dist/<os>-<arch>/free-meal-cli)
make build
...
./free-meal-cli list category
./free-meal-cli list filter

# 5. Override the API base URL for a single call
FREE_MEAL_BASE=https://api.example.com ./free-meal-cli list category

# 6. No arguments -> interactive REPL
./free-meal-cli
free-meal> list category
free-meal> /quit
```
...
1. **Build the binary.** From this `go-cli/` directory:
   ...
   ```sh
   make build          # -> dist/<os>-<arch>/free-meal-cli
   ```
...
3. **Run a query.** Evaluate an boru expression against the API (or run with no
   ...
   ```sh
   ./dist/*/free-meal-cli list category
   ```
...
Run with no arguments to open a REPL (prompt `free-meal>`). Each line is
...
| Target | Result |
|--------|--------|
| `make build` | Native binary at `dist/<os>-<arch>/free-meal-cli`. |
...
(`./free-meal-cli <expr>`) and interactively (the REPL), and expressions compose the
