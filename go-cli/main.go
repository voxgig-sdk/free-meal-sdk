// Command FreeMeal-cli is a CLI + REPL for the FreeMeal SDK,
...
const prompt = "free-meal"

// entitiesHelp is the space-separated entity list shown by /help.
const entitiesHelp = "category filter latest list lookup random randomselection search"
...
func usage(out io.Writer) {
	...
	fmt.Fprintln(out, "Env:      FREE_MEAL_APIKEY (api key), FREE_MEAL_BASE (base url override)")
...
func run(args []string, in io.Reader, out, errOut io.Writer) int {
	...
	// Configure from the environment: FREE_MEAL_APIKEY carries the API key and
	// FREE_MEAL_BASE optionally overrides the API base URL (e.g. production).
	// Both injectable by a secrets vault. Unset -> nil config defaults.
	var opts map[string]any
	if apikey := os.Getenv("FREE_MEAL_APIKEY"); apikey != "" {
		opts = map[string]any{"apikey": apikey}
	}
	if base := os.Getenv("FREE_MEAL_BASE"); base != "" {
		if opts == nil {
			opts = map[string]any{}
		}
		opts["base"] = base
	}
	client := sdk.NewFreeMealSDK(opts)
